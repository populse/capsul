...
from soma.qt_gui.qt_backend.QtWebEngineCore import (
    ...
    def __new__(cls, name, bases, dict):
        for k, v in dict.items():
            if callable(v) and v.__annotations__:
                args = [
                    type for name, type in v.__annotations__.items() if name != "return"
                ]
                ...
                result._params = [
                    name for name in v.__annotations__ if name != "return"
                ]
...
class WebHandler:
    ...
    def __init__(
        self, capsul, base_url, server_type, routes, backend, static, **kwargs
    ):
        """
...
class CapsulHTTPHandlerMeta(type(http.server.BaseHTTPRequestHandler)):
    ...
    def __new__(
        ...
        if name != "CapsulHTTPHandler":
            ...
            if missing:
                raise TypeError(
                    f'CapsulHTTPHandlerMeta.__new__() missing {len(missing)} required positional arguments: {", ".join(missing)}'
                )
            if capsul is None:
...
class CapsulHTTPHandler(
    http.server.BaseHTTPRequestHandler, metaclass=CapsulHTTPHandlerMeta
):
    """
    ...
    def do_GET(self):
        ...
        if template_data is None:
            ...
            if data is None:
                ...
                header["Content-Type"] = mime_type
                header["Last-Modified"] = self.date_time_string(
                    os.stat(template.filename).st_mtime
                )
...
class CapsulBrowserWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        self, starting_url="capsul://dashboard", capsul=None, routes=None, backend=None
    ):
        ...
        if not QWebEngineUrlScheme.schemeByName(b"capsul").name():
            ...
            capsul.url_scheme_handler = CapsulSchemeHandler(
                None, capsul=capsul, routes=routes, backend=backend
            )
        ...
        self.channel.registerObject(
            "backend", capsul.url_scheme_handler._handler.backend
        )
