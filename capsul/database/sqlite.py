...
class SQliteExecutionDatabase(ExecutionDatabase):
    ...
    def _connect(self):
        ...
        try:
            ...
            if sqlite.execute(sql).fetchone()[0] == 0:
                sqlite.execute(sql)
                sql = (
                    "CREATE TABLE capsul_properties ( "
                    "key TEXT PRIMARY KEY, "
                    "value TEXT )"
                )
                sqlite.execute(sql)
                sql = (
                    "CREATE TABLE capsul_engine ( "
                ...
                sqlite.execute(sql)
                sql = (
                    "CREATE TABLE capsul_connection ( "
                    "connection_id TEXT PRIMARY KEY, "
                    "date TEXT )"
                )
                sqlite.execute(sql)
                sql = (
                    "CREATE TABLE capsul_execution ( "
                    "engine_id TEXT, "
                    "execution_id TEXT, "
                    "label TEXT, "
                    "status TEXT, "
    ...
    def get_or_create_engine(self, engine, update_database=False):
        with self._read_write() as sqlite:
            ...
            if row:
                ...
                engine_id = str(uuid4())
                sql = (
                    "INSERT INTO capsul_engine (engine_id, label, config, "
        ...
        with self._read() as sqlite:
            if sqlite:
                ...
                if row:
                    ...
                    for execution_id in executions:
                        sql = (
                            "SELECT label, status, waiting, ready, ongoing, done, failed "
        ...
        with self._read_write() as sqlite:
            ...
            if row:
                ...
                sqlite.executemany(
                    sql,
                    [
                        [engine_id, execution_id, job["uuid"], json.dumps(job)]
                        for job in jobs
                    ],
                )
            return execution_id
    ...
    def job_finished_json(
        ...
        with self._read_write() as sqlite:
            ...
            sql = (
                "UPDATE capsul_execution SET ready=?, ongoing=?, failed=?, "
    ...
    def execution_report_json(self, engine_id, execution_id):
        with self._read() as sqlite:
            if sqlite:
                sql = (
                    "SELECT label, status, tmp, error, error_detail, start_time, end_time, "
    ...
    def dispose(self, engine_id, execution_id):
        with self._read_write() as sqlite:
            ...
            if row[0] == "ended":
                sql = (
                    "SELECT persistent, executions FROM capsul_engine WHERE engine_id=?"
                )
                row = sqlite.execute(sql, [engine_id]).fetchone()
    ...
    def end_execution(self, engine_id, execution_id):
        with self._read_write() as sqlite:
            ...
            if dispose:
                ...
                sqlite.execute(sql, [engine_id, execution_id])
                sql = (
                    "DELETE FROM capsul_execution WHERE engine_id=? AND execution_id=?"
                )
                sqlite.execute(sql, [engine_id, execution_id])
