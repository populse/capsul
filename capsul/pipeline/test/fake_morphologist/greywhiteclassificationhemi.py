...
class GreyWhiteClassificationHemi(Process):
    def __init__(self, **kwargs):
        ...
        self.add_field("commissure_coordinates", File, read=True, allowed_extensions=['.APC'], write=False)
        ...
        self.add_field("lesion_mask_mode", Literal['e','w','g'], optional=True)
        self.lesion_mask_mode = 'e'
        self.add_field("grey_white", File, write=True, allowed_extensions=['.nii.gz', '.bmp', '.dcm', '', '.i', '.v', '.fdf', '.gif', '.ima', '.dim', '.jpg', '.mnc', '.nii', '.pbm', '.pgm', '.png', '.ppm', '.img', '.hdr', '.tiff', '.tif', '.vimg', '.vinfo', '.vhdr', '.xbm', '.xpm', '.mnc.gz'], read=True)
