...
class AcpcOrientation(Process):
    def __init__(self, **kwargs):
        ...
        self.add_field(
            "commissure_coordinates", File, write=True, extensions=[".APC"], read=True
        )
        self.add_field(
            "Normalised",
            Literal["No", "MNI from SPM", "MNI from Mritotal", "Marseille from SPM"],
        )
        self.Normalised = "No"
        self.add_field(
            "Anterior_Commissure",
            pydantic.conlist(float, min_items=3, max_items=3),
            optional=True,
