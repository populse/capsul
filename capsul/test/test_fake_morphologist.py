...
from capsul.pipeline.test.fake_morphologist.morphologist import Morphologist
...
from capsul.pipeline.test.fake_morphologist.normalization_aimsmiregister import (
    normalization_aimsmiregister,
)
from capsul.pipeline.test.fake_morphologist.normalization_fsl_reinit import (
    Normalization_FSL_reinit,
)
from capsul.pipeline.test.fake_morphologist.t1biascorrection import T1BiasCorrection
from capsul.pipeline.test.fake_morphologist.histoanalysis import HistoAnalysis
from capsul.pipeline.test.fake_morphologist.brainsegmentation import BrainSegmentation
from capsul.pipeline.test.fake_morphologist.skullstripping import skullstripping
from capsul.pipeline.test.fake_morphologist.scalpmesh import ScalpMesh
from capsul.pipeline.test.fake_morphologist.splitbrain import SplitBrain
from capsul.pipeline.test.fake_morphologist.greywhiteclassificationhemi import (
    GreyWhiteClassificationHemi,
)
from capsul.pipeline.test.fake_morphologist.greywhitetopology import GreyWhiteTopology
from capsul.pipeline.test.fake_morphologist.greywhitemesh import GreyWhiteMesh
from capsul.pipeline.test.fake_morphologist.pialmesh import PialMesh
from capsul.pipeline.test.fake_morphologist.sulciskeleton import SulciSkeleton
from capsul.pipeline.test.fake_morphologist.sulcigraph import SulciGraph
from capsul.pipeline.test.fake_morphologist.sulcilabellingann import SulciLabellingANN
from capsul.pipeline.test.fake_morphologist.sulcilabellingspamglobal import (
    SulciLabellingSPAMGlobal,
)
from capsul.pipeline.test.fake_morphologist.sulcilabellingspamlocal import (
    SulciLabellingSPAMLocal,
)
from capsul.pipeline.test.fake_morphologist.sulcilabellingspammarkov import (
    SulciLabellingSPAMMarkov,
)
from capsul.pipeline.test.fake_morphologist.sulcideeplabeling import SulciDeepLabeling


normalization_t1_spm12_reinit.requirements = {"fakespm": {"version": "12"}}


class SPM12NormalizationBIDS(
    ProcessSchema, schema="bids", process=normalization_t1_spm12_reinit
):
...
class SPM12NormalizationBrainVISA(
    ...
    normalized_anatomy_data = {"analysis": undefined, "prefix": "normalized_SPM"}


class SPM12NormalizationShared(
    ProcessSchema, schema="shared", process=normalization_t1_spm12_reinit
):
    anatomical_template = {"data_id": "normalization_template"}


normalization_t1_spm8_reinit.requirements = {"fakespm": {"version": "8"}}


class SPM8NormalizationBIDS(
    ProcessSchema, schema="bids", process=normalization_t1_spm8_reinit
):
...
class SPM8NormalizationShared(
    ProcessSchema, schema="shared", process=normalization_t1_spm8_reinit
):
    anatomical_template = {"data_id": "normalization_template"}


class AimsNormalizationBIDS(
    ProcessSchema, schema="bids", process=normalization_aimsmiregister
):
    transformation_to_ACPC = {"part": "normalized_aims", "extension": "trm"}
...
class AimsNormalizationShared(
    ProcessSchema, schema="shared", process=normalization_aimsmiregister
):
    anatomical_template = {"data_id": "normalization_template"}
...
class FSLNormalizationBrainVISA(
    ...
    transformation_matrix = {"analysis": undefined, "suffix": "fsl", "extension": "mat"}
...
class GreyWhiteMeshBrainVISA(ProcessSchema, schema="brainvisa", process=GreyWhiteMesh):
    _ = {
        "*": {"seg_directory": "segmentation/mesh"},
    }
...
class SulciSkeletonBrainVISA(ProcessSchema, schema="brainvisa", process=SulciSkeleton):
    _ = {
        "*": {"seg_directory": "segmentation"},
    }
...
class SulciGraphBrainVISA(ProcessSchema, schema="brainvisa", process=SulciGraph):
    ...
    cortex_mid_interface = {"seg_directory": "segmentation", "prefix": "gw_interface"}
...
class MorphologistBrainVISA(ProcessSchema, schema="brainvisa", process=Morphologist):
    _ = {
...
class TestFakeMorphologist(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.capsul = Capsul(
            "test_fake_morphologist", site_file=self.config_file, user_file=None
        )
        return super().setUp()
    ...
    def test_path_generation(self):
        ...
        for it, normalization in enumerate(zip(sel_tal, renorm, norm, normspm)):
            ...
            metadata = ProcessMetadata(
                morphologist, execution_context, datasets=datasets
            )
    ...
    def test_pipeline_iteration(self):
        ...
        metadata = ProcessMetadata(
            morphologist_iteration, execution_context, datasets=datasets
        )
        metadata.bids = iter_meta
        print("ITER:", morphologist_iteration.iterative_parameters)
        print(
            "left_labelled_graph is path:",
            morphologist_iteration.field("left_labelled_graph").path_type,
        )
        ...
        print(
            "left_labelled_graph:",
            getattr(morphologist_iteration, "left_labelled_graph", undefined),
        )
