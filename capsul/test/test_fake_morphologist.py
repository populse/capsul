...
class TestFakeMorphologist(unittest.TestCase):
    ...
    def test_pipeline_iteration(self):
        ...
        morphologist_iteration.metadata_schema = {
            'brainvisa': {
                '*': {
                    'analysis': '!{{executable.select_Talairach[{list_index}]}}-{{executable.Normalization_select_Normalization_pipeline[{list_index}]}}',
                }
            }
        }
        ...
        generate_paths(morphologist_iteration, execution_context,
                       source_fields=['t1mri'], datasets=datasets, debug=False)
