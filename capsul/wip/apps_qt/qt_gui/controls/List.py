...
from capsul.apps_qt.resources import icones
...
class ListControlWidget(object):
    ...
    def create_widget(parent, control_name, control_value, trait):
        ...
        controller_widget = ControllerWidget(controller, parent=frame, live=True) 
        ...
        # Resize callback
        resize_hook = partial(ListControlWidget.expand_or_collapse, frame)
    ...
    def update_controller(controller_widget, control_name, control_instance,
        ...
        control_instance: QFrame (mandatory)
            ...
            with the controller
        """
        # Check if the control values are valid before updating the controller
        if ListControlWidget.is_valid(control_instance):
    ...
    def update_controller_widget(cls, controller_widget, control_name,
        ...
        if control_name in controller_widget.controller.user_traits():
            ...
            # Get the number of list elements in the controller associated with the
            # current list control
            len_widget = len(control_instance.controller.user_traits())

            # Parameter that is True if a user trait associated with the current
            # list control has changed
            ...
            if len(trait_value) < len_widget:
                ...
                # Notify that some traits of the inner list controller have been
                # deleted
                user_traits_changed = True

            # Special case: some new traits have been added to the top controller
            elif len(trait_value) > len_widget:

                # Need to add to the inner list controller some traits with type
                # 'inner_trait'
    ...
    def connect(cls, controller_widget, control_name, control_instance):
        ...
        if not control_instance.connected:
            ...
            for inner_control_name, inner_control in inner_controls.iteritems():
    ...
    def delete_one_row(control_instance, index_to_remove):
        ...
        widget_item = control_instance.controller_widget._grid_layout.itemAtPosition(
            index_to_remove, 1)
        ...
        if widget_item is not None:
            ...
            control_instance.controller_widget._grid_layout.removeItem(widget_item)
            widget.deleteLater()

            # Try to get the widget label in column one
            label_item = control_instance.controller_widget._grid_layout.itemAtPosition(
                index_to_remove, 0)
            ...
            if label_item is not None:
                ...
                control_instance.controller_widget._grid_layout.removeItem(label_item)
                label.deleteLater()
    ...
    def delete_list_item(controller_widget, control_name, control_instance):
        ...
        if item_found:
            ...
            # Indeed, two elements are inserted for a list item (tools + widget)
    ...
    @staticmethod
    def expand_or_collapse(control_instance):
