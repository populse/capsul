    ...
    with capsul_engine.settings as settings:
        settings.ensure_module_fields(
            ...
            [
                ...
                dict(name="config", type="str", description="path of fsl.sh file"),
                dict(
                    name="prefix",
                    type="str",
        ...
        if not config:
            settings.new_config(
                "fsl", "global", {capsul_engine.settings.config_id_field: "fsl"}
            )
...
def check_configurations():
    ...
    """
    fsl_prefix = capsul.engine.configurations.get("capsul.engine.module.fsl", {}).get(
        "prefix", ""
    )
    fsl_config = capsul.engine.configurations.get("capsul.engine.module.fsl", {}).get(
        "config"
    )
    if not fsl_config:
...
def edition_widget(engine, environment, config_id="fsl"):
    ...
    controller.add_trait(
        "directory",
        traits.Directory(traits.Undefined, desc="Directory where FSL is installed"),
    )
    controller.add_trait(
        "config",
        traits.File(
            traits.Undefined, output=False, desc="Parameter to specify the fsl.sh path"
        ),
    )
    controller.add_trait(
        "prefix", traits.String(traits.Undefined, desc="Prefix to add to FSL commands")
    )
