...
def edition_widget(engine, environment, config_id="any"):
    ...
    def validate_config(widget):
        ...
        with widget.engine.settings as session:
            ...
            id = "spm%s%s" % (
                controller.version if controller.version != traits.Undefined else "",
            ...
            if conf is None:
                ...
                for k in ("directory", "standalone", "version"):
                    if k == "directory" and values[k] and not os.path.isdir(values[k]):
            ...
            if id != widget.config_id:
                try:
                    session.remove_config("spm", widget.environment, widget.config_id)
                except Exception:
                    pass
                widget.config_id = id
    ...
    controller.add_trait(
        "directory",
        traits.Directory(
            traits.Undefined, output=False, desc="Directory containing SPM."
        ),
    )
    controller.add_trait(
        "standalone",
        traits.Bool(False, desc="If True, use the standalone version of SPM."),
    )
    controller.add_trait(
        "version",
        traits.Str(
    ...
    if config_id == "any":
        ...
        try:
            ...
            pass
    if conf:
        controller.directory = conf.get("capsul.engine.module.spm", {}).get(
            "directory", traits.Undefined
        )
        controller.standalone = conf.get("capsul.engine.module.spm", {}).get(
            "standalone", False
        )
        controller.version = conf.get("capsul.engine.module.spm", {}).get(
            "version", traits.Undefined
        )
        config_id = conf.get("capsul.engine.module.spm", {}).get("config_id", config_id)
