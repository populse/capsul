...
def edition_widget(engine, environment, config_id="any"):
    ...
    def validate_config(widget):
        ...
        with widget.engine.settings as session:
            ...
            conf = session.config("somaworkflow", widget.environment, selection=query)
            if conf is None:
            ...
            if id != widget.config_id:
                try:
                    session.remove_config(
                        "somaworkflow", widget.environment, widget.config_id
                    )
                except Exception:
                    pass
                widget.config_id = id
    ...
    controller.add_trait(
        "config_file", traits.File(traits.Undefined, output=False, desc="")
    )
    controller.add_trait(
        "keep_failed_workflows", traits.Bool(True, output=False, desc="")
    )
    controller.add_trait(
    ...
    if config_id == "any":
        ...
        try:
            ...
            pass
    if conf:
        controller.use = conf.get("capsul.engine.module.somaworkflow", {}).get(
            "use", True
        )
        controller.computing_resource = conf.get(
            "capsul.engine.module.somaworkflow", {}
        ).get("computing_resource", traits.Undefined)
        controller.config_file = conf.get("capsul.engine.module.somaworkflow", {}).get(
            "config_file", traits.Undefined
        )
        config_id = conf.get("capsul.engine.module.somaworkflow", {}).get(
            "config_id", config_id
        )
        ...
        controller.queue = conf.get("capsul.engine.module.somaworkflow", {}).get(
            "queue", Undefined
        )
        ...
        if (
            ...
            with engine.settings as session:
                ...
                params = {
                    ...
                    "somaworkflow_keep_failed_workflows": "keep_failed_workflows",
                    "somaworkflow_keep_succeeded_workflows": "keep_succeeded_workflows",
                }
                values = {}
            ...
            with engine.settings as session:
                ...
                if config:
                    params = {
                        ...
                        "computing_resource": "somaworkflow_computing_resource",
                        "config_file": "somaworkflow_config_file",
                        "keep_failed_workflows": "somaworkflow_keep_failed_workflows",
                        "keep_succeeded_workflows": "somaworkflow_keep_succeeded_workflows",
                    }
