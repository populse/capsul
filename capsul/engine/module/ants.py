...
def init_settings(capsul_engine):
    with capsul_engine.settings as settings:
        ...
        if not config:
            settings.new_config(
                "ants", "global", {capsul_engine.settings.config_id_field: "ants"}
            )
...
def edition_widget(engine, environment, config_id="ants"):
    ...
    controller.add_trait(
        "directory",
        traits.Directory(traits.Undefined, desc="Directory where ANTS is installed"),
    )

    conf = engine.settings.select_configurations(environment, {"ants": "any"})
    if conf:
